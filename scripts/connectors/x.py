...
MOCK_X_AUTH_BASE_URL = "http://localhost/mock-oauth/x"
...
def load_x_config(env: dict[str, str] | None = None) -> XConfig:
    values = env if env is not None else dict(os.environ)
    return XConfig(
        clientId=_env_value(values, "X_CLIENT_ID"),
        clientSecret=_env_value(values, "X_CLIENT_SECRET"),
        redirectUri=_env_value(values, "X_REDIRECT_URI"),
        realOAuthEnabled=_env_truthy(values.get("X_ENABLE_REAL_OAUTH")),
        realPublishingEnabled=_env_truthy(values.get("X_ENABLE_REAL_PUBLISHING")),
    )
...
class XConnector(SocialConnector):
        ...
        super().__init__(
            ...
            requiredScopes=(
                PlatformPermissionScope(
                    id="users.read",
                    label="Read profile",
                    description="Placeholder scope for future X profile discovery. TODO: verify before production.",
                    status=PlatformFeatureStatus.SCAFFOLDED,
                ),
                PlatformPermissionScope(
                    id="tweet.read",
                    label="Read posts",
                    description="Placeholder scope for future readiness checks. Reading timelines/comments is disabled in this build.",
                    required=False,
                ...
                PlatformPermissionScope(
                    id="tweet.write",
                    label="Write posts",
                    description="Placeholder scope for future posting. Publishing is disabled in this build.",
                    required=False,
                    status=PlatformFeatureStatus.PLANNED,
                ),
                PlatformPermissionScope(
                    ...
                    description="Placeholder scope for future token refresh support. TODO: verify access and pricing before production.",
                    required=False,
                    status=PlatformFeatureStatus.PLANNED,
                ),
            ),
            setupInstructions=(
                "Create or use an X developer app before future real OAuth testing.",
                "Add the local redirect URI, for example http://localhost:8000/api/connect/x/callback.",
                "Set X_CLIENT_ID, X_CLIENT_SECRET, and X_REDIRECT_URI in local .env only.",
                "API access/pricing warning: X API tiers, product access, limits, and pricing can change and must be reviewed before real work.",
                "Required scopes are placeholders: users.read for profile checks and tweet.write for future posting. TODO: verify exact scopes before production.",
                "App review warning: X may require product access or review before live posting permissions work.",
    ...
    def getOAuthConfig(self) -> OAuthConfig:
        ...
        return OAuthConfig(
            ...
            status=status,
            notes="X OAuth is scaffolded. Mock mode is default and publishing remains disabled.",
        )
    ...
    def buildAuthorizationUrl(self, request: OAuthStartRequest) -> OAuthStartResult:
        ...
        if mode == "mock":
            return OAuthStartResult(
                ...
                authorizationUrl=_mock_authorization_url(
                    redirect_uri=redirect_uri or "http://localhost:8000/api/connect/x/callback",
                    state=state,
                    scopes=tuple(scopes),
                ),
                status="mock_redirect_ready",
                message="Mock X authorization URL created. No real X API was called.",
            )
        ...
        if config.missingConfigKeys or not redirect_uri:
            return OAuthStartResult(
                ...
                message=(
                    "X OAuth setup is missing required local environment variables: "
        ...
        if not config.realOAuthEnabled:
            return OAuthStartResult(
                success=False,
                status="setup_required",
                message="Real X OAuth is disabled. Keep mock mode unless a future task enables guarded OAuth.",
            )
        ...
        return OAuthStartResult(
            ...
            message="Guarded X authorization URL built locally. Token exchange and publishing remain disabled.",
        )
    ...
    def handleOAuthCallback(self, request: OAuthCallbackRequest) -> OAuthCallbackResult:
        if request.error:
            return OAuthCallbackResult(
                ...
                message="X OAuth returned an error. No token exchange was attempted.",
            )
        ...
        return OAuthCallbackResult(
            ...
            message="X token exchange is scaffolded only. Mock OAuth remains the default path.",
        )
    ...
    def validateConnection(
        ...
        **kwargs: Any,
        ...
        response = PlatformHttpClient(
            http_client_config
            or PlatformHttpClientConfig(provider="x", platform=self.platform, allowNetwork=False)
        ).request(_profile_request())
        if not response.ok:
            result = _health_from_failed_response(account, response, checked_at=checked_at, debug=debug)
            _record_x_health(db_path, result)
            return result

        payload = response.json if isinstance(response.json, dict) else {}
        profile = _extract_profile(payload)
        platform_account_id = profile.get("id") or account["platform_account_id"] or f"mock-x-{account['id']}"
        display_name = profile.get("displayName") or account["display_name"] or "Mock X Account"
        username = profile.get("username") or account["username"]
        status = "healthy"
        warnings: list[str] = []
        if response.mocked and not profile:
            warnings.append("mock_discovery: Mock X profile was synthesized locally.")
        elif not profile:
            status = "limited"
        ...
        result = ConnectorHealthResult(
            ...
            warnings=warnings,
            message="X profile health checked through mock/scaffold discovery.",
    ...
    def publishText(self, payload: dict[str, Any] | None = None) -> ConnectorActionResult:
        return _x_publishing_disabled("text")

    def publishImage(self, payload: dict[str, Any] | None = None) -> ConnectorActionResult:
        return _x_publishing_disabled("image")

    def publishVideo(self, payload: dict[str, Any] | None = None) -> ConnectorActionResult:
        return _x_publishing_disabled("video")


def _x_publishing_disabled(content_type: str) -> ConnectorActionResult:
    return ConnectorActionResult(
        success=False,
        status="disabled_by_policy",
        message=(
            f"X {content_type} publishing is disabled by policy. "
...
def _mock_authorization_url(*, redirect_uri: str, state: str, scopes: tuple[str, ...]) -> str:
    ...
    return f"{MOCK_X_AUTH_BASE_URL}/authorize?{query}"
...
def _health_from_failed_response(
    ...
    *,
    ...
    return ConnectorHealthResult(
        ...
        errors=[provider_error.userSafeMessage if provider_error else "X health check failed safely."],
        retryable=bool(provider_error and provider_error.retryable),
        message=provider_error.userSafeMessage if provider_error else "X health check failed safely.",
        rawProviderResponseRedacted=_redacted_response_text(response) if debug else None,
    )


def _record_x_health(db_path: Path, result: ConnectorHealthResult) -> None:
    ...
    create_connector_audit_log(
        ...
        message=result.message or "X connector health check completed locally.",
        safe_metadata={
            "connectionStatus": result.connectionStatus,
            "requiresReauth": result.requiresReauth,
            "warnings": result.warnings,
